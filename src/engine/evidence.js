...
function buildEvidence(sd) {
  if (!sd) return [];
  const { fluid, cCoeff, orifice, selection, backpress } = sd;
  return [
    {
      id:1, title:"유체 물성 확인",
    ...
    {
      id:3, title:"필요 오리피스 면적",
      formula:"A = (W / (C × Kd_eff × Kb)) × √(T×Z / (M×P1²))",
      result:`A_required = ${orifice.areaCm2.toFixed(4)} cm²`,
      detail:`W=${orifice.W} kg/h, P1=${orifice.P1} barg, Kd_eff=${orifice.KdEff.toFixed(3)}, Kb=${orifice.Kb}${orifice.isRD ? ` (럽처디스크 보정계수 ×${API_CONST.RD_KD_FACTOR} 적용)` : ""}`,
      ok: orifice.areaCm2 > 0,
    },
    {
      id:4, title:"API 526 표준 오리피스 선정",
    ...
    {
      id:5, title:"Back Pressure 검증",
      formula:`P2/P1 < ${(API_CONST.BACKPRESSURE_SPRING*100).toFixed(0)}% (스프링식) / ${(API_CONST.BACKPRESSURE_PILOT*100).toFixed(0)}% (파일럿식)`,
      result:`P2/P1 = ${(backpress.ratio*100).toFixed(1)}% → ${backpress.ratio < backpress.limitSpring ? "스프링식 적합 ✓" : backpress.ratio < backpress.limitPilot ? "파일럿식 검토 필요 ⚠" : "초과 ✗"}`,
